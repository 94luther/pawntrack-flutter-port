...
class PawnTrackApi {
  ...
  Future<void> inventorySale(
    ...
    final saleId = _stableId(
        '${item['sheetName'] ?? 'inventory'}-row-${item['rowNumber'] ?? item['id']}-sale');
  ...
  Future<Map<String, dynamic>> uploadItemPhoto({
    ...
    return _uploadFile(
        kind: 'item-photo', itemId: itemId, bytes: bytes, fileName: fileName);
  }
  ...
  Future<Map<String, dynamic>> _uploadFile({
    ...
    final path = switch (kind) {
      'customer-photo' =>
        'customers/${customerId ?? 'unassigned-customer'}/customer-photo/$fileId-$safeName',
      'id-photo' =>
        'customers/${customerId ?? 'unassigned-customer'}/id-photo/$fileId-$safeName',
      'item-photo' =>
        'items/${itemId ?? 'unassigned-item'}/photos/$fileId-$safeName',
      'proof-of-ownership' =>
        'items/${itemId ?? 'unassigned-item'}/proof/$fileId-$safeName',
    ...
    if (customerId != null &&
        (kind == 'customer-photo' || kind == 'id-photo')) {
    ...
    if (itemId != null &&
        (kind == 'item-photo' || kind == 'proof-of-ownership')) {
      await firestore.collection('items').doc(itemId).set({
        kind == 'item-photo' ? 'itemPhotoUrls' : 'proofOfOwnershipUrls':
  ...
  Future<void> _savePayload(Map<String, dynamic> payload,
      ...
      required List<Map<String, dynamic>> updates}) async {
    final next = {
      ...payload,
  ...
  Future<void> _recordOperationalEvent(
      Map<String, dynamic> metadata, List<Map<String, dynamic>> updates) async {
    if (metadata['type'] == null) return;
    final actor = metadata['actor'] is Map
        ? Map<String, dynamic>.from(metadata['actor'] as Map)
        : <String, dynamic>{};
    final actorUid = '${actor['uid'] ?? ''}'.trim();
    await firestore.collection('auditLog').add(_clean({
          'entityType': metadata['type'],
          'entityId': metadata['loan']?['rowNumber'] ??
              metadata['item']?['rowNumber'] ??
    ...
    if (actorUid.isNotEmpty) {
      await firestore.collection('staffUsers').doc(actorUid).set(
          _clean({
            ...actor,
            'lastAction': metadata['type'],
            'lastSeenAt': FieldValue.serverTimestamp(),
            'updatedAt': FieldValue.serverTimestamp(),
          }),
          SetOptions(merge: true));
      await firestore.collection('userActivityLog').add(_clean({
            'userId': actorUid,
    ...
    if (metadata['type'] == 'loan_update' &&
        _num(metadata['loan']?['paymentAmount']) > 0) {
    ...
    if (metadata['type'] == 'new_pawn' || metadata['type'] == 'new_loan') {
      await firestore.collection('voiceCommands').add(_clean({
            'transcript': metadata['type'] == 'new_loan'
                ? 'Create a new loan for ${metadata['customerName'] ?? 'customer'}'
                : 'Create a new pawn for ${metadata['customerName'] ?? 'customer'}',
            'parsedAction': metadata['type'],
            'status': 'applied',
            'payload': metadata,
            'createdAt': FieldValue.serverTimestamp(),
          }));
    ...
    if (metadata['type'] == 'forfeiture') {
      await firestore.collection('auditLog').add(_clean({
            'entityType': 'loan',
            'entityId':
                '${metadata['loan']?['sheetName']}:${metadata['loan']?['rowNumber']}',
  ...
  Future<void> _mirrorRows(Map<String, dynamic> payload) async {
    ...
    _mirrorLoans(
        batch, 'Active Pawns', payload['activePawns'] as List<dynamic>? ?? []);
    _mirrorLoans(batch, 'OS Debts', payload['osDebts'] as List<dynamic>? ?? []);
    _mirrorInventory(
        batch, payload['companyOwnedItems'] as List<dynamic>? ?? []);
    await batch.commit();
  }
  ...
  void _mirrorLoans(WriteBatch batch, String sheetName, List<dynamic> rows) {
    for (final row in toObjects(rows)) {
      ...
      final loanId = _stableId(
          '${sheetName == 'Active Pawns' ? 'active-pawns' : 'os-debts'}-row-$rowNumber');
      final customerId = _stableId(
          '${row['Customer ID Number / Omang'] ?? row['Omang'] ?? row['Client Number'] ?? row['Phone Number'] ?? row['Client Name'] ?? loanId}');
      final loanAmount = _num(row['Loan Amount']);
      final interestAmount = _num(row['Interest Amount']);
      final totalPayback = _num(row['Total Payback']) != 0
          ? _num(row['Total Payback'])
          : loanAmount + interestAmount;
      final amountPaid = _num(row['Amount Paid']);
      final remainingBalance = _num(row['Remaining Balance']) != 0
          ? _num(row['Remaining Balance'])
          : (totalPayback - amountPaid).clamp(0, double.infinity);
      ...
      final riskScore =
          ((overdueDays > 0 ? (overdueDays * 1.5).clamp(0, 45) : 0) +
      ...
      batch.set(
          ...
          SetOptions(merge: true));
      batch.set(
          firestore.collection('loans').doc(loanId),
          _clean({
            'sheetName': sheetName,
            'rowNumber': rowNumber,
            'customerId': customerId,
          ...
          SetOptions(merge: true));
      batch.set(
          firestore.collection('riskScores').doc(loanId),
          _clean({
            'customerId': customerId,
            'loanId': loanId,
            'score': riskScore,
  ...
  void _mirrorInventory(WriteBatch batch, List<dynamic> rows) {
    for (final row in toObjects(rows)) {
      ...
      final profit = _num(row['Profit/loss']) != 0
          ? _num(row['Profit/loss'])
          : (sold > 0 ? sold - pawnedAmount : 0);
      batch.set(
          firestore.collection('inventory').doc(inventoryId),
          _clean({
            ...
            'listDate':
                parseDate(row['List Date'])?.toIso8601String().substring(0, 10),
            ...
            'saleDate':
                parseDate(row['Sale Date'])?.toIso8601String().substring(0, 10),
            'dateGiven': parseDate(row['Date Given'])
                ?.toIso8601String()
                .substring(0, 10),
  ...
  List<List<dynamic>> _deepCloneRows(List<dynamic> rows) {
    return rows
        .map((row) => row is List ? List<dynamic>.from(row) : <dynamic>[row])
        .toList();
  }

  void _applySheetUpdate(
      Map<String, dynamic> payload, Map<String, dynamic> update) {
    ...
    for (var rowOffset = 0; rowOffset < values.length; rowOffset++) {
      ...
      final row = rows[rowIndex] is List
          ? rows[rowIndex] as List<dynamic>
          : <dynamic>[rows[rowIndex]];
      rows[rowIndex] = row;
  ...
  _ParsedRange _parseRange(String range) {
    final match = RegExp(r"^'?([^']+)'?!([A-Z]+)(\d+)(?::([A-Z]+)(\d+))?$",
            caseSensitive: false)
        .firstMatch(range);
  ...
  Map<String, dynamic> _clean(Map<String, dynamic> value) {
    return Map<String, dynamic>.fromEntries(
      ...
      if (v is Map<String, dynamic>) {
        return MapEntry(entry.key, _clean(v));
      }
      if (v is Map) {
        return MapEntry(entry.key, _clean(Map<String, dynamic>.from(v)));
      }
      if (v is List) {
        return MapEntry(
            entry.key,
            v
                .map((item) => item is Map
                    ? _clean(Map<String, dynamic>.from(item))
                    : item)
                .toList());
      }
      return entry;
